...
tarch::la::Vector<DIMENSIONS_TIMES_TWO, bool> peanoclaw::mappings::SolveTimestep::fillBoundaryLayers(
  ...
  if ((fineGridVertices[fineGridVerticesEnumerator(1)].isBoundary() || !tarch::la::allGreater(fineGridVerticesEnumerator.getVertexPosition(1), _domainOffset)
      || !tarch::la::allGreater(_domainOffset + _domainSize, fineGridVerticesEnumerator.getVertexPosition(1)))
  ...
  if ((fineGridVertices[fineGridVerticesEnumerator(4)].isBoundary() || !tarch::la::allGreater(fineGridVerticesEnumerator.getVertexPosition(4), _domainOffset)
      || !tarch::la::allGreater(_domainOffset + _domainSize, fineGridVerticesEnumerator.getVertexPosition(4)))
...
void peanoclaw::mappings::SolveTimestep::prepareSendToNeighbour(
  peanoclaw::Vertex& vertex, int toRank,
...
void peanoclaw::mappings::SolveTimestep::prepareCopyToRemoteNode(
  peanoclaw::Vertex& localVertex, int toRank,
  const tarch::la::Vector<DIMENSIONS, double>& x,
  const tarch::la::Vector<DIMENSIONS, double>& h,
  int level
) {
...
void peanoclaw::mappings::SolveTimestep::prepareCopyToRemoteNode(
  peanoclaw::Cell& localCell, int toRank,
...
bool peanoclaw::mappings::SolveTimestep::prepareSendToWorker(
  ...
  if (_estimatedRemainingIterationsUntilGlobalTimestep.find(worker)
      == _estimatedRemainingIterationsUntilGlobalTimestep.end()) {
...
//  } else if((_estimatedRemainingIterationsUntilGlobalTimestep[worker]) / 2 <= 1) {
//    logInfo("", "Reducing");
//  } else {
//    logInfo("", "Avoiding 1");
//  }
...
void peanoclaw::mappings::SolveTimestep::prepareSendToMaster(
  peanoclaw::Cell& localCell, peanoclaw::Vertex * vertices,
...
void peanoclaw::mappings::SolveTimestep::mergeWithMaster(
  ...
  assertion1(
      _estimatedRemainingIterationsUntilGlobalTimestep.find(worker)
          != _estimatedRemainingIterationsUntilGlobalTimestep.end(), worker);
  _estimatedRemainingIterationsUntilGlobalTimestep[worker] = std::max(1,
    workerState.getEstimatedNumberOfIterationsToGlobalTimestep()
  );

  //TODO unterweg debug
//  std::cout << "Estimated iterations on worker " << worker << ": " << workerState.getEstimatedNumberOfIterationsToGlobalTimestep() << std::endl;
...
void peanoclaw::mappings::SolveTimestep::touchVertexFirstTime(
  ...
  if (fineGridVertex.shouldRefine()
      && fineGridVertex.getRefinementControl()
          == peanoclaw::Vertex::Records::Unrefined) {
    //TODO unterweg debug
...
//    );
  ...
  } else if (fineGridVertex.shouldErase()
      && fineGridVertex.getRefinementControl()
          == peanoclaw::Vertex::Records::Refined
...
//    Patch& subgrid = fineGridCell.getSubgrid();
    ...
    if (fineGridCell.isLeaf()) {
      ...
      //Perform timestep
      double maximumTimestepDueToGlobalTimestep = _globalTimestepEndTime
      ...
      assertion2(!tarch::la::smaller(subgrid.getTimeIntervals().getCurrentTime() + subgrid.getTimeIntervals().getTimestepSize(), endTime),
          subgrid.getTimeIntervals().getCurrentTime() + subgrid.getTimeIntervals().getTimestepSize(), endTime);

#ifdef Asserts
      if (subgrid.containsNaN()) {
        logError("", "Invalid solution in patch " << subgrid.toString() << std::endl << subgrid.toStringUNew() << std::endl << subgrid.toStringUOldWithGhostLayer());
        ...
        for (int i = 0; i < TWO_POWER_D; i++) {
          ...
          for (int j = 0; j < TWO_POWER_D; j++) {
            if (fineGridVertices[fineGridVerticesEnumerator(i)].getAdjacentCellDescriptionIndex(j) != -1) {
              CellDescription& cellDescription = CellDescriptionHeap::getInstance().getData(
                      fineGridVertices[fineGridVerticesEnumerator(i)].getAdjacentCellDescriptionIndex(j)).at(0);
              Patch neighborPatch(cellDescription);
              logError("", i << " " << j << std::endl << neighborPatch.toString() << std::endl << neighborPatch.toStringUNew() << std::endl << neighborPatch.toStringUOldWithGhostLayer());
            } else {
...
void peanoclaw::mappings::SolveTimestep::leaveCell(
  ...
  if (tarch::la::oneGreater(patch.getSubcellSize(), tarch::la::Vector<DIMENSIONS, double>(patch.getDemandedMeshWidth()))) {
    ...
    for (int i = 0; i < TWO_POWER_D; i++) {
      ...
      tarch::la::Vector<DIMENSIONS, double>(patch.getDemandedMeshWidth()))) {
...
void peanoclaw::mappings::SolveTimestep::beginIteration(
  ...
  if (!tarch::la::equals(_globalTimestepEndTime,
      solverState.getGlobalTimestepEndTime())) {
    ...
    for (std::map<int, int>::iterator i = _estimatedRemainingIterationsUntilGlobalTimestep.begin();
        i != _estimatedRemainingIterationsUntilGlobalTimestep.end(); i++) {
      i->second = 1;
    }
  }
...
//    }
  ...
  for(std::map<int,int>::iterator i = _estimatedRemainingIterationsUntilGlobalTimestep.begin();
      ...
      i++) {
    solverState.setEstimatedNumberOfIterationsToGlobalTimestep(
      std::max(solverState.getEstimatedNumberOfIterationsToGlobalTimestep(), i->second)
    );
  }
#endif
